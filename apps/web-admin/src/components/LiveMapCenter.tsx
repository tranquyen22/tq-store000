...
const mockSOSList: SOSSignal[] = [
  {
    ...
    senderRole: 'DRIVER',
    locationName: '456 Lê Duẩn, Quận 1',
    ...
    <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm mb-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-700 mb-6">
        <h2 className="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
          ...
          <span>Bản Đồ Điều Hành Realtime & Trung Tâm Cảnh Báo SOS</span>
        </h2>
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        {/* Realtime Live Map Simulation Canvas */}
        <div className="lg:col-span-7 h-64 rounded-2xl bg-gradient-to-br from-slate-900 via-sky-950 to-slate-900 p-4 border border-slate-800 relative overflow-hidden flex flex-col justify-between text-white">
          ...
          <div className="relative z-10 flex justify-between items-center text-xs">
            <span className="bg-slate-900/80 px-2.5 py-1 rounded-lg border border-slate-700 font-bold flex items-center gap-1">
              <Radio className="w-3 h-3 text-emerald-400 animate-ping" /> Live PostGIS Radar Active
            </span>
            <span className="text-slate-400">Cập nhật mỗi 2 giây</span>
          </div>

          <div className="relative z-10 bg-slate-900/90 p-3 rounded-xl border border-slate-800 backdrop-blur-md text-xs space-y-1">
            <strong className="text-sky-400 block">Vùng Nóng Nhu Cầu Cao (Hotspots):</strong>
            <p className="text-slate-300">📍 Quận 1 (Phố đi bộ Nguyễn Huệ) - Nhu cầu đặt xe tăng 180%</p>
          </div>
        </div>

        {/* Emergency SOS Control Center Panel */}
        <div className="lg:col-span-5 bg-slate-50 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col">
          <h3 className="font-extrabold text-xs text-red-600 dark:text-red-400 uppercase mb-3 flex items-center gap-1.5">
            ...
            <span>Trung Tâm Báo Động Khẩn Cấp SOS ({sosList.filter(s => s.status === 'PENDING').length})</span>
          </h3>
          ...
          <div className="flex-1 space-y-3 max-h-48 overflow-y-auto pr-1">
            {sosList.map(sos => (
              <div key={sos.id} className={`p-3 rounded-xl border text-xs ${sos.status === 'PENDING' ? 'bg-red-500/10 border-red-500/40 text-red-700 dark:text-red-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                ...
                {sos.status === 'PENDING' ? (
                  <button onClick={() => handleResolveSOS(sos.id)} className="w-full py-1.5 bg-red-600 hover:bg-red-700 text-white font-extrabold text-[11px] rounded-lg">
                    ĐÃ XỬ LÝ & BẢO VỆ AN TOÀN
                  </button>
                ) : (
                  <span className="text-emerald-500 font-bold flex items-center gap-1 text-[11px]"><CheckCircle2 className="w-3.5 h-3.5" /> Đã xử lý sự cố</span>
                )}
              </div>
            ))}
          </div>
