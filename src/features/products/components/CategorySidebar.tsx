...
export const CategorySidebar: React.FC<CategorySidebarProps> = ({
  categories, selectedCategory, onSelectCategory, maxPrice, setMaxPrice, onReset
}) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl p-6 shadow-sm sticky top-24">
      <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700 mb-6">
        ...
        <button onClick={onReset} className="text-xs font-semibold text-sky-600 dark:text-sky-400 flex items-center gap-1 hover:underline">
          <RotateCcw className="w-3.5 h-3.5" /><span>Đặt lại</span>
        </button>
      </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-semibold text-sm transition-all ${
                  isActive ? 'bg-gradient-to-r from-sky-500 to-blue-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-300 hover:bg-sky-50 dark:hover:bg-slate-700/50'
                }`}
              >
                {getIcon(cat.id)}
      ...
      <div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center text-xs font-bold text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-slate-700/50 p-2.5 rounded-xl border">
            <span>Đến:</span><span>{formatVND(maxPrice)}</span>
          </div>
          <input
            type="range" min="50000" max="50000000" step="50000"
            value={maxPrice}
